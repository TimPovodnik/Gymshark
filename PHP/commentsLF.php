<?php
    ...
    // INSERT KOMENTARJA Z OCENO V BAZO
    if(isset($_POST['commentSubmit'])) { // Če kliknemo na gumb comment zažene kodo
      ...
      if($uid != $row3['uporabnik_id'] AND $pid != $row3['program_id']) {    
        ...
        if (!mysqli_stmt_prepare($stmt2, $sql2)) {
                       header("Location: ../loseFat.php?error=sqlerror2"); // Če stavek ne dela
                       exit();
               }
               else {        
    ...
    function getComments($conn) {
            $sql = "SELECT * FROM komentarji WHERE program_id = 1;"; // dobimo vse komentarje iz baze kjer je program_id = 1 (Komentarji s id = 1 spadajo na stran LoseFat.php)
             ...
             if($row2 = mysqli_fetch_assoc($result2)) { // v $row2 shranimo rezultat, ki ga dobimo iz SQL stavka
                 echo "<div class='row'>";
                  echo "<div class='col-md-12'>";
                       ...
                       // OCENE
                       $sql3 = "SELECT * FROM Ocene WHERE uporabnik_id='$id' AND program_id = 1;"; // SELECTA VSE IZ TABELE OCENE KJER JE ID ENAK IDJU KI NAPIŠE KOMENTAR
                       $result3 = mysqli_query($conn, $sql3); // IZVEDE v BAZI
                       $row3 = mysqli_fetch_assoc($result3);
                            if( $row3['ocena'] == 0) {
                       ...
                       $_SESSION['userName'] = $row2['u_ime']; // Da lahko displayam prijavljenega uporabnika, če kdaj želim izpisati User Name uproabnika
                ...
                if(isset($_SESSION['userId'])) { // preveri če smo loginani
                       if($_SESSION['userId'] == $row['uporabnik_id'] || $_SESSION['userAdminId'] == true) { // preveri če se $_SESSION['USerId'] ( id loged-in uporabnika) ujema z uporabnik_id-jem tistega ki je komentiral (iz tabele uporabniki) // Če je admin lahko edita in deleta komentarje vseh uporabnikov.
                           echo "<form  method='POST' action='PHP/commentsLF.php'>  
                               <input type='hidden' name='id' value='".$row['id']."'> 
                               <button class='delete-btn' type='submit' name='commentDeleteSubmit'>Delete</button>
                        </form>
                        <form  method='POST' action='commentEditLF.php'> <!-- Vzamemo podatke in jih poslemo na drugo stran, kejr jih editamo --->
                            <input type='hidden' name='id' value='".$row['id']."'> 
    ...
    if(isset($_POST['commentEditSubmit'])) { // Če kliknemo na gumb update zažene kodo
        ...
        $sql3 = "UPDATE Ocene SET ocena = ? WHERE uporabnik_id = '$uid' AND program_id = 1;";
        ...
        if (!mysqli_stmt_prepare($stmt3, $sql3)) {
               ...
               header("Location: ../loseFat.php?commentedit=success");
               exit();
        }
    }
    ...
    // DELETE KOMENTARJA
       if(isset($_POST['commentDeleteSubmit'])) { // Če kliknemo na gumb update zažene kodo
        ...
        if (!mysqli_stmt_prepare($stmt3, $sql3)) {
               ...
               header("Location: ../loseFat.php?commentdelete=success");
               exit();
           }
    }
